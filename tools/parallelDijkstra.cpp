...
// Function to perform Dijkstra's algorithm for single-source shortest path in parallel
std::vector<std::vector<Edge>> dijkstraParallel(const CSRMatrix& graph, int source) {
    ...
    while (!pq.empty()) {
        ...
        // Traverse the adjacent nodes of the current node in parallel
...
// Function to print the CSR matrix representation
...
int main(int argc, char** argv) {
    ...
    // Perform Dijkstra's algorithm in parallel
    std::vector<std::vector<Edge>> shortestPaths = dijkstraParallel(csrMatrix, sourceNode);
