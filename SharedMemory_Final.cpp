    ...
    while (1)
    {
        ...
        for (int n = 0 ; n < affectedNodesN.size(); n++)
        {
            ...
            {   
                ...
                {
                    if (affectedNodesList[predecessor[n][j].source - 1] == 1)
                    {
                        affectedNodesList[predecessor[n][j].source - 1] = 0;
                        if (affectedNodesDel[predecessor[n][j].source - 1] == 1)
                        {   
                            affectedNodesDel[predecessor[n][j].source - 1] = 0;
                            if ( n + 1 == 1)
                                continue;
                            int newDistance = std::numeric_limits<double>::infinity();
                            int newParentIndex = -1; 
                            ...
                            for (int k = 0; k < ssspTree[n].second.size(); k++)
                            {
                                ...
                                affectedNodesDel[ssspTree[n].second[k] - 1] = 1; 
                            }
                        }
                        int v = predecessor[n][j].source - 1;
                        int w = predecessor[n][j].weight;
                        if (shortestDist[n] > shortestDist[v] + w && shortestDist[v] != std::numeric_limits<double>::infinity()) 
                        {    
                            shortestDist[n] = shortestDist[v] + w;
                            int oldParent = parentList[n + 1];
                            for (int j = 0; j < ssspTree[oldParent - 1].second.size(); j++ )
                            {
                                if (ssspTree[oldParent - 1].second[j] == n + 1 )
